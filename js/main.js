...
function Articulo(id, nombre, precio, tamanio, destacado, imagen) {
  this.id = id;
  this.nombre = nombre;
  this.precio = precio;
...
function generarHtmlProducto(producto) {
  var html = `<div class="col-sm col-md-6 col-xl-4 bot1 cartas">
...
function cargarSelect(producto) {
  var option = `<option value="${producto.id}">${producto.nombre} ${producto.tamanio}</option>`;
  $("#gourmet").append(option);
}


function agregarPrecio() {
  $("#error").html("");
  var valor = $("#gourmet option:selected").val();
...
function agregarProducto() {
  ...
  if (cantidad > 0) {
    $("#error").html("");
    var itemId = parseInt($("#gourmet").val());
...
function finalizarPedido() {
  ...
  $("#error-cliente").html("");
  var mensaje = `Muchas gracias por tu compra ${$("#name").val()}, estaremos enviando tu pedido a ${$("#adress").val()} en breve`;
